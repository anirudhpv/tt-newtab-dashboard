...
async function sendChat() {
  ...
  if (!apiKey) { appendMsg('assistant', 'Add your Anthropic API key in settings.'); return; }
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'Content-Type':'application/json', 'x-api-key':apiKey, 'anthropic-version':'2023-06-01' },
      body: JSON.stringify({ model:'claude-3-haiku-20240307', max_tokens:1024, messages })
    });
...
document.getElementById('theme-btn').addEventListener('click', () => {
  isDark = !isDark;
  document.documentElement.style.setProperty('--ink', isDark ? '#0A1119' : '#f2f6fa');
  document.documentElement.style.setProperty('--surface', isDark ? '#1B2126' : '#ffffff');
  document.documentElement.style.setProperty('--text', isDark ? '#e8edf2' : '#0d1c28');
