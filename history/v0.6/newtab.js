// v0.6 — marked.js + DOMPurify: full markdown rendering in chat responses
        ...
        document.getElementById('weather').innerHTML = `
          <div style="display:flex;align-items:center;gap:12px">
            ...
            <div>
              ...
              <div style="font-size:12px;color:#7a92a4">${esc(data.weather[0].description)}</div>
...
async function sendChat() {
  ...
  if (!apiKey) { appendMsg('assistant', 'Add your Z.ai API key in settings.'); return; }
  ...
  try {
    const res = await fetch('https://api.z.ai/api/coding/paas/v4/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role:'system', content:'You are a concise AI assistant for a performance marketing agency.' },
          ...messages
        ]
      })
    });
  ...
  if (role === 'assistant') {
    ...
    if (sources.length) {
      sources.slice(0, 3).forEach(s => {
        ...
        a.innerHTML = `<img src="https://www.google.com/s2/favicons?domain=${d}&sz=16" width="12" height="12" alt=""><span>${esc(s.title || d)}</span>`;
        el.appendChild(a);
      });
    }
  } else {
...
function appendThinking() {
  ...
  el.innerHTML = '<span style="font-style:italic;color:#7a92a4;font-size:11px">Thinking…</span>';
  c.appendChild(el);
  c.scrollTop = c.scrollHeight;
  return el;
}
